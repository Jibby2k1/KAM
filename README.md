...
The quality-scale campaign passed preflight and Stage 1 frontier extraction, then retained 20/32 valid Wave 1 rows when 12 rows hit the original three-hour infrastructure limit. A selective timeout repair is queued from array `38087856` through final report job `38087863`; it preserves completed evidence and reruns only missing rows with corrected memory/retrieval budgets. See [`docs/codex/KAM_PHASE6_OVERNIGHT_TIMEOUT_REPAIR.md`](docs/codex/KAM_PHASE6_OVERNIGHT_TIMEOUT_REPAIR.md). This is a queued experiment, not a result.
