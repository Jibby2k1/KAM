...
### Phase VI parameter-dynamics follow-up

The matched mechanism study is [`docs/codex/KAM_PHASE6_PARAMETER_DYNAMICS_PLAN.md`](docs/codex/KAM_PHASE6_PARAMETER_DYNAMICS_PLAN.md), with its machine-readable contract at [`configs/phase6/parameter_dynamics_v1.yaml`](configs/phase6/parameter_dynamics_v1.yaml). It holds decoder shape, memory supports, expert form, initialization, data order, and token budget constant while varying only key trainability, freeze timing, and joint versus alternating optimization. The required figures track parameter-group displacement, key update rates, layerwise drift, routing stability, and the relationship between parameter movement and validation change.

The 10-row instrumentation pilot is complete with decision `PILOT_PASS`; its local report and plots are under [`reports/phase6/parameter_dynamics_v1/pilot/`](reports/phase6/parameter_dynamics_v1/pilot/). The 60-row main was not submitted. A broader proposed replacement is the [`Phase 6.2 behavioral atlas and solution-space plan`](docs/codex/KAM_PHASE6_BEHAVIORAL_ATLAS_V2_PLAN.md), with machine-readable contract at [`configs/phase6/behavioral_atlas_v2.yaml`](configs/phase6/behavioral_atlas_v2.yaml). It adds immutable anchor banks, optimizer-provenance correction, windowed gradient/update statistics, query-versus-key routing decomposition, functional rank and Jacobian measures, causal interventions, expressivity scaling, and prequential adaptation tests. Stage 0 is implemented and locally validated; the final bounded NVIDIA L4 profile passed all audits. See [`reports/phase6/behavioral_atlas_v2/l4_profile_r3/`](reports/phase6/behavioral_atlas_v2/l4_profile_r3/) and the gate record in the plan. The original 24-row noninferential Stage 0 completed 23 rows; its compile profile failed in a PyTorch 2.8 CUDA Graph allocator path, so the preserved decision is `STAGE0_BLOCKED`. The revisioned measurement repair and dependency-gated 168-row Stage 1 implementation are documented in [`KAM_PHASE6_BEHAVIORAL_ATLAS_STAGE0_REPAIR_STAGE1_HANDOFF.md`](docs/codex/KAM_PHASE6_BEHAVIORAL_ATLAS_STAGE0_REPAIR_STAGE1_HANDOFF.md). The repaired audit passed on HPG and the dependency-gated 168-row Stage 1 array `38385305` is running at four-way L4 occupancy; final report job `38385306` remains dependency-held.

The outcome-to-action rules frozen before complete Stage 1 inspection are in [`KAM_PHASE6_STAGE1_CONDITIONAL_DECISION_MEMO.md`](docs/codex/KAM_PHASE6_STAGE1_CONDITIONAL_DECISION_MEMO.md). They require complete validity, Holm-controlled paired effects, formal ±1% equivalence testing, checkpoint-reuse causal evidence, and a systems gate before Stage 2 promotion.
