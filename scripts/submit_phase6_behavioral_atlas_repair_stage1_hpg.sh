...
REPAIR_ROOT="${PHASE6_ATLAS_REPAIR_ROOT:-${BASE}/results/phase6/behavioral_atlas_v2/stage0_measurement_repair_r1}"
REPAIR_REPORT_ROOT="${PHASE6_ATLAS_REPAIR_REPORT_ROOT:-${BASE}/reports/phase6/behavioral_atlas_v2/stage0_measurement_repair_r1}"
REPAIR_LOG_ROOT="${PHASE6_ATLAS_REPAIR_LOG_ROOT:-${BASE}/logs/phase6/behavioral_atlas_v2/stage0_measurement_repair_r1}"
...
stage1_smoke="$(sbatch --parsable --account="${HPG_ACCOUNT}" --qos="${HPG_QOS}" --partition="${HPG_PARTITION}" --time=01:00:00 --array=123 --dependency="afterok:${repair_report}" --export="${stage1_smoke_export}" --output="${STAGE1_LOG_ROOT}/smoke_%A_%a.out" --error="${STAGE1_LOG_ROOT}/smoke_%A_%a.err" "${HPG_REPO}/slurm/phase6_behavioral_atlas_stage1_array.sbatch")"
stage1_array="$(sbatch --parsable --account="${HPG_ACCOUNT}" --qos="${HPG_QOS}" --partition="${HPG_PARTITION}" --time=08:00:00 --array=0-167%4 --dependency="afterok:${stage1_smoke}" --export="${stage1_export}" --output="${STAGE1_LOG_ROOT}/row_%A_%a.out" --error="${STAGE1_LOG_ROOT}/row_%A_%a.err" "${HPG_REPO}/slurm/phase6_behavioral_atlas_stage1_array.sbatch")"
stage1_report="$(sbatch --parsable --account="${HPG_ACCOUNT}" --qos="${HPG_QOS}" --partition="${HPG_PARTITION}" --dependency="afterany:${stage1_array}" --export="${stage1_export}" --output="${STAGE1_LOG_ROOT}/final_%j.out" --error="${STAGE1_LOG_ROOT}/final_%j.err" "${HPG_REPO}/slurm/phase6_behavioral_atlas_stage1_final.sbatch")"
